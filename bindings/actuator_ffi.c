...
extern bool spark_validate_command(double current, double target, double max_rate, double max_def, double min_def);
extern command_t spark_apply_limits(command_t cmd, limits_t limits);
extern bool spark_is_within_limits(double value, limits_t limits);
...
extern bool spark_is_consensus(command_t c1, command_t c2, command_t c3, double tolerance);
extern double spark_get_current_deflection(int id);
extern double spark_get_temperature(int id);
extern double spark_get_hydraulic_pressure(int id);
extern int spark_get_overall_status(int id);
extern bool spark_allocate_power(int id, double required_power);
