...
let test_optimizer_compute () =
  ...
  Alcotest.(check (float 0.5)) "first deflection near target" 1.0
    (List.hd cmds).deflection
...
let test_mode_manager () =
  ...
  let new_state = transition state AutoPilot false in
  Alcotest.(check bool) "mode transition" true (get_current_mode new_state = AutoPilot);
  let cmd = { deflection = 0.5; rate_limit = 0.5 } in
  let processed, _ = process_command new_state cmd in
  Alcotest.(check (float 0.001)) "rate limited" 0.1 processed.rate_limit;
  let emergency, _ = emergency_override_command new_state 0.0 in
  Alcotest.(check (float 0.001)) "emergency rate" 0.01 emergency.rate_limit

let test_binding () =
  let lim = { max_deflection = 0.9; min_deflection = -0.9 } in
  let cmd = { target_deflection = 0.5; rate_limit = 0.1 } in
  let valid = validate_command 0.0 cmd.target_deflection cmd.rate_limit
      lim.max_deflection lim.min_deflection in
  Alcotest.(check bool) "validate command" true valid;
  let applied = apply_limits cmd lim in
  Alcotest.(check (float 0.001)) "apply limits" 0.5 applied.target_deflection;
  ...
  Alcotest.(check (float 0.001)) "majority vote median" 0.5 voted.target_deflection

let () =
  let open Alcotest in
  run "Actuator Engine Tests" [
    "optimizer", [ test_case "compute trajectory" `Quick test_optimizer_compute ];
    "setpoint_gen", [ test_case "generate setpoints" `Quick test_setpoint_gen ];
    "mode_manager", [ test_case "mode transitions" `Quick test_mode_manager ];
    "binding", [ test_case "FFI functions" `Quick test_binding ];
  ]
