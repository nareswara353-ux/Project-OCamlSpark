...
let test_full_flow () =
  ...
  let bridge_cmd = {
    target_deflection = first_cmd.deflection;
    rate_limit = first_cmd.rate_limit;
  } in
  let valid =
...
let test_redundancy_integration () =
  ...
  Alcotest.(check (float 0.001)) "voted median" 0.5 voted.target_deflection;
...
let test_emergency_integration () =
  ...
  Alcotest.(check (float 0.001)) "emergency deflection" 0.0 emergency_cmd.deflection

let () =
  let open Alcotest in
  run "Integration Tests" [
    "full_flow", [ test_case "complete trajectory to command" `Quick test_full_flow ];
    "redundancy", [ test_case "voter and consensus" `Quick test_redundancy_integration ];
    "emergency", [ test_case "emergency override" `Quick test_emergency_integration ];
  ]
